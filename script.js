...
window.uploadColumn = async function () {
  ...
  // ===============================
  // MEDIA MODE — re-inject a range that was captured with Extract
  // (text + any images/audio/video packed into those cells)
  // ===============================
  if (direction === "media") {
    let payload;
    try {
  ...
  // ===============================
  // PLAIN TEXT MODE (original behavior, unchanged)
  // ===============================
...
// ===============================
// EXTRACT RANGE (text + media aware)
// ===============================
// Converts a blob: URL (from URL.createObjectURL) into a base64 data: URL so
// the file itself — not just a page-local pointer — travels with the text
// when a range is extracted, pasted elsewhere, or re-uploaded later.
async function blobUrlToDataUrl(url) {
  if (!url) return null;
  if (url.startsWith("data:")) return url; // already portable
  try {
...
async function extractRange(mode) {
  ...
  let extracted = [];   // plain tab-separated text (used when no media present)
  let grid = [];         // structured text + media, used for media-aware re-upload
  let mediaCount = 0;
  ...
  for (let r = startRow; r <= endRow; r++) {
    ...
    for (let c = startCol; c <= endCol; c++) {
      ...
      // Pull any media (image/audio/video) packed into this cell
      let mediaList = [];
  ...
  if (mode === "copy") {
    if (mediaCount > 0) {
      // Media can't ride along on the system clipboard as plain text, so a
      // media-containing extraction is loaded straight into the Upload box
      // (Direction is switched to "Use File (Media Column)") ready to be
      // re-injected into a new range with every file intact.
      ...
      // No media in range — plain clipboard copy, exactly as before
